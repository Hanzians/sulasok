...
def add_notification(message):
    ...
    notifications_list.append(full_message)
...
def add_inventory():
    global item_name_entry, quantity_entry, category_entry, price_entry, unit_combobox, inv_treeview
    ...
    if item and quantity and category and price:
        ...
        if cursor:
            ...
            add_notification(f"Inventory item added: {item}")
            load_inventory_data()
...
def clear_inventory_fields():
    global item_name_entry, quantity_entry, category_entry, price_entry, unit_combobox
...
def notification_df():
    ...
    undolabel.place(x=5, y=2)
    undolabel.image = undo_icon
    listbox = Listbox(notif_frame, font=('Times', 12), width=120, height=30)
    listbox.place(x=100, y=100)
    for notif in notifications_list:
        listbox.insert(END, notif)
    def clear_notifs():
        listbox.delete(0, END)
        notifications_list.clear()
    Button(notif_frame, text="Clear Notifications", font=('times new roman', 15), bg='red', fg='white',
           command=clear_notifs).place(x=100, y=500)
